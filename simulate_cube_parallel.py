...
def f(N,a_eff_cta_south,data_bg_rate,psf_cta_south,num_slices,time_per_slice,bins_,cu_min,cu_max,trans_pos_ra,trans_pos_dec,transient_templates, list_templates):
    ...
    slices_transient, trans_scale, ra_transient, dec_transient = simulate_steady_source_with_transient(
                    ...
                    transient_template=transient_templates[list_templates[N]],
                    num_slices=num_slices,
                    time_per_slice=time_per_slice * u.s,
                    bins=[bins_, bins_],
                    )
    ...
    return slices, trans_scales, list_cu_flare,list_ra_transient,list_dec_transient
...
# Choose start of transient dependent on template
    ...
    liste = []
    '''
    with Pool(processes=2) as pool:         # start x worker processes
        maxi = n_transient
        with tqdm(total=maxi) as pbar:
            for i, res in (enumerate(pool.imap_unordered(partial(f,
                a_eff_cta_south= a_eff_cta_south,
                data_bg_rate = data_bg_rate,
                psf_cta_south=psf_cta_south,
                num_slices = num_slices,
                time_per_slice = time_per_slice,
                bins_ = bins_,
                cu_min = cu_min,
                cu_max = cu_max,
                trans_pos_ra = trans_pos_ra,
                trans_pos_dec = trans_pos_dec),range(2)))):
                    pbar.update()
                    #liste.append(res)

    slices = liste[n_transient-1][0]
    trans_scale = liste[n_transient-1][1]
    list_cu_flare = liste[n_transient-1][2]
    list_ra_transient = liste[n_transient-1][3]
    list_dec_transient = liste[n_transient-1][4]
    '''
    for i in tqdm(range(n_transient)):
        ...
        a_eff_cta_south= a_eff_cta_south,
            ...
            cu_max = cu_max,
            trans_pos_ra = trans_pos_ra,
            trans_pos_dec = trans_pos_dec,
            transient_templates = transient_templates,
